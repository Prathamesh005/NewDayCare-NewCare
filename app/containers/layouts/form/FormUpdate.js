  ...
  switch (fieldConfig.type) {
        ...
        <Grid item xs={12} sm={12}>
          ...
          >
            {fieldConfig.options.map(option => (
              <FormControlLabel
                value={option.value}
                control={<Checkbox color="primary" checked={fieldConfig.checked?fieldConfig.checked:false}/>}
    ...
    case 'search':
        ...
        <Grid item xs={12} sm={fieldConfig.size}>
         ...
         <div style={{marginTop:'5'}}>{avaliableOT&&avaliableOT!=null?avaliableOT.map(c=>c.location.name):'No OTS Are Avaliable'}</div>
       </Grid>:null}
       </>
      );
      break;
