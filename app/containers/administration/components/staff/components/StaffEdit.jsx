...
const StaffEdit = props => {
  ...
  let phone = '';
  let email = '';
  let role = { code: '', display: '' };
  let language = { code: '', display: '' };
  let qualification = { code: '', display: '' };
  let experienceYear = { code: '', display: '' };
  let registrationDetails = '';
  let channelLink = '';
  let signatureText = '';

  if (props.location.state && props.location.state.detailData) {
    const allData = props.location.state.detailData.practitioner;

    firstName = allData['first'];
    middleName = allData['middle'];
    lastName = allData['last'];
    ...
    experienceYear = allData['experienceYear'];
    registrationDetails = allData['registrationDetails'];
    channelLink = allData['channelLink'];
    signatureText = allData['signatureText'];
  }
  ...
  const onSave = async (values, { resetForm }) => {
    ...
    setSaveLoader(true);
    // const { payload } = await props.doPractionerSave(practionerEdit(field));
    setSaveLoader(false);
