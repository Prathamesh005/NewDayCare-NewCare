...
function CreateAppointmentFormFromEdit(props) {
  //-------------- using thunk ----------------
  useAppointmentSlice();
  //-------------- using thunk ----------------
  ...
  const onSave = (values, { resetForm }) => {
    const obj = {
      ...
      appointmentTimeSlot: backDateCheck(values, isBackDatedAllowed)
        ? null
        : values.appointmentTimeSlot || null,
      comment: values.comment,
      instructions: values.instructions,
      vip: values.vip,
      typeOfAppointment: values.typeOfAppointment,
    };
    console.log("detailsForEditAppointment", props.detailsForEditAppointment)
    setIsLoading(true);
    // debugger
    // callCreateAppointmentAPI(obj);
    props.actionType && props.actionType === "update" ? callUpdateAppointmentAPI(values) : callCreateAppointmentAPI(obj);
  };
  const callUpdateAppointmentAPI = async values => {
    const patientDetails = {
      ...props.patientData,
      serviceType: (props.detailsForEditAppointment && props.detailsForEditAppointment.serviceType) || null,
    };
    const obj = {
      appointmentResourceId: props.detailsForEditAppointment && props.detailsForEditAppointment.appointmentResourceId,
      appointmentStatus: props.detailsForEditAppointment && props.detailsForEditAppointment.appointmentStatus,
    ...
    const { payload } = await props.updateAppointment(obj);
    setIsLoading(false);
    if (payload.data) {
      if (
        history.location &&
        history.location.state &&
        history.location.state.detailsForEditAppointment &&
        history.location.state.detailsForEditAppointment.detaildata &&
        selectedSlot.slot.start === 'instant' &&
        docData &&
        docData.userDetails &&
        docData.userDetails.userRole === 'Doctor'
      ) {
        props.snackbarShowMessage(props.editAppointmentSuccess, 'success');
        setTimeout(function () {
          history.push({
            ...
            >
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={8}>
                    <Autocomplete
                      ...
                      size="small"
                      disabled={props.disableAppointmentFormExist}
                      ...
                      renderInput={params => (
                        <TextField
                          ...
                          onChange={e => getPractitionerDataFromAPI(e)}
                          helperText={
              ...
              <Grid item xs={12}>
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={4}>
                    {/* {selectedPractitioner === null || selectedPractitioner === undefined || Object.keys(selectedPractitioner).length === 0 ?
                                        <DatePickerField name="appointmentDate"
                                            type="date"
                  ...
                  <Grid item xs={4}>
                    <Autocomplete
                      ...
                      getOptionLabel={option =>
                        option && option.slot && option.slot.start == 'instant'
                          ? 'Instant'
                          : option &&
                            option.slot &&
                            option.slot.start.split('+')[1] === '05:30'
...
export function mapDispatchToProps(dispatch) {
  return {
    ...
    updateAppointment: fieldValues => dispatch(updateAppointment(fieldValues)),
